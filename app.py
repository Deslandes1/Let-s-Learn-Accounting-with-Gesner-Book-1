...
# ---------- Fallback translation dictionary (for common terms) ----------
...
def translate_text(text, dest_lang):
    """Translate text using available library, or fallback to dictionary."""
    if not text or dest_lang == "en":
        return text

    # First, try using the translation library
    if TRANSLATOR_AVAILABLE:
        try:
            ...
            pass  # fallback to dictionary

    # Fallback: use the hard-coded dictionary for common terms
    fallback = FALLBACK_TRANSLATIONS.get(dest_lang, {})
    for eng, translated in fallback.items():
        # Replace whole words (case-insensitive) to avoid partial matches
        text = re.sub(r'\b' + re.escape(eng) + r'\b', translated, text, flags=re.IGNORECASE)
    return text

def translate_symbols(symbols_str, dest_lang):
    """Translate symbols string, preserving structure."""
    if not symbols_str or dest_lang == "en":
        return symbols_str
    parts = symbols_str.split('|')
    translated_parts = []
    for part in parts:
        part = part.strip()
        # If part is only emojis or special chars, keep as is
...
def translate_lesson_content(lesson_dict, dest_lang, lesson_num):
    if not TRANSLATOR_AVAILABLE and dest_lang not in FALLBACK_TRANSLATIONS:
        return lesson_dict
    if dest_lang == "en":
        return lesson_dict
...
def get_lesson_data(lesson_num, lang_code):
    ...
    lesson_en = lessons.get(lesson_num, lessons[1])
